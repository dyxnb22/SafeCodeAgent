...
> 历史文件路径描述的是当时的改动清单，不保证对应文件仍在当前
> docs 树中。旧的 `docs/version-notes/v*.md` 已合并到
> [release-ledger.md](release-ledger.md)，完整旧内容可从 git 历史查看。
...
| `v3.6.5` ✅ | `main` | `docs/why-safecode.md`, `docs/why-safecode.md`, `docs/troubleshooting.md` | T-3.6.5-A landing-docs: adds three landing docs; claims aligned with `docs/public-contracts.md`; experimental surfaces labeled throughout; all three linked from `README.md`; 20 new tests in `tests/test_landing_docs.py`; documentation only, no runtime changes |
...
Next active plan: `docs/planning-history.md` (streaming, shell polish, live connectivity, fuzzy matching, per-patch undo, agent-loop transparency).

Readiness baseline: historical v3.11.x commercial readiness audit, now kept in
git history rather than the current docs tree.
...
Completed. Plan: `docs/planning-history.md` (COMPLETED as of v4.18.1).
...
| `v5.1.2` ✅ | `dev/v4.19` | `README.md`, `docs/user-guide.md`, `docs/troubleshooting.md`, `docs/security/threat-model-v3.6.md`, `tests/test_mvp_docs.py` | Trust modes docs cut: README "Trust Modes (v5.1)" section with mode comparison table; MVP guide "Trust Modes" section with auto-edit walkthrough, full-auto CI example, and session rollback; troubleshooting "Trust Mode Issues" section covering undo-all, file count guard, full-auto unexpected command, Ctrl-C abort, and command-delay-ms; threat model v5.1.0 addendum (trust-mode surface table, mitigations, what does NOT change); 5 doc guard tests. 5208 passed, 4 skipped. |
...
| `v5.2.2` ✅ | `dev/v4.19` | `README.md`, `docs/user-guide.md`, `docs/troubleshooting.md`, `tests/test_mvp_docs.py` | Display docs cut: README "Shell Display (v5.2)" section with prompt format, /cost, compact diff, and output collapsing. MVP guide "Understanding What the Agent Did" section with prompt anatomy, /cost breakdown, diff header, and output collapse example. Troubleshooting "Display Issues" section covering /cost not showing price (mock vs no data), full command output (audit log / JSON mode), and session edit summary. 4 doc guard tests; 5242 passed, 4 skipped. |

## v5.3.x: Context Intelligence

| 版本 | 分支 | 主要入口 | 验收命令 / 结果 |
|---|---|---|---|
| `v5.3.2` ✅ | `dev/v4.19` | `README.md`, `docs/user-guide.md`, `docs/troubleshooting.md`, `docs/current-status.md`, `tests/test_mvp_docs.py` | Context intelligence docs cut + v5.3 baseline: README "Context Intelligence (v5.3)" section with import-graph, git context, compaction; MVP guide "Working with Large Codebases" with import seeding walkthrough, git context, compaction archive + threshold config; troubleshooting "Context Intelligence Issues" (wrong files → pin/seed, compaction archives, extra tokens); project-final-status.md updated to v5.3.2 baseline with B-series table, capability table, deferred items; 5 doc guards; 5284 passed, 4 skipped. |
| `v5.3.1` ✅ | `dev/v4.19` | `src/safecode/context/compaction.py`, `src/safecode/agent/loop.py`, `tests/test_context_compaction.py` | Context compaction: `ContextCompactor(llm_client, project_root, session_id)` with `should_compact(token_estimate)` at 60% threshold; `compact(observations)` calls model.ask() with summarisation prompt; archives originals to `.sac/sessions/<id>/observations_compacted_N.jsonl`; emits `tool_call_compact` audit event; returns `CompactionResult`. `AgentLoop._session_observations` accumulates turn observations; `_maybe_compact_context()` checks threshold and triggers compaction; prints `[Context compacted: N → M tokens]` notice; compact summary injected into next `native_step()` context. Lazy `_get_compactor()` init. 14 tests; 5279 passed, 4 skipped. |
| `v5.3.0` ✅ | `dev/v4.19` | `src/safecode/context/git_context.py`, `src/safecode/index/import_graph.py`, `src/safecode/context/collector.py`, `src/safecode/agent/orchestrator.py`, `tests/test_import_graph.py` | Git-aware context: `GitContextCollector` runs git log/diff with shell=False, timeout=10s; `GitContext.to_context_block()` renders ≤2000-char block with recent commits, branch-diff files, high-churn files; graceful failure when git unavailable. Import-graph seeding: `ImportGraph` parses Python (ast.parse), TypeScript/JS (regex), Go (regex) imports; `first_degree_imports(seed, limit=5)` resolves imports to project-relative paths; `ContextCollector.collect(seed_files=[...], include_git_context=True)` adds `import_context` and `git_context` keys. `AgentOrchestrator.edit()`: `_extract_seed_files(task)` extracts file paths from task text; passes seed_files + include_git_context=True to collector. 23 tests; 5265 passed, 4 skipped. |
...
| 版本 | 分支 | 主要入口 | 验收命令 / 结果 |
|---|---|---|---|
| `v5.4.0` ✅ | `dev/v4.19` | `src/safecode/mcp/native_bridge.py`, `src/safecode/agent/loop.py`, `src/safecode/cli_mcp.py`, `tests/test_mcp_native_bridge.py` | MCP native tool bridge: `MCPNativeToolBridge(runner, server)` wraps MCP server's read-class tools as `NativeToolSpec` instances with `mcp_<server>_<tool>` prefix; `discover_and_register(dispatcher)` registers handlers that call `MCPReadOnlyRunner.call_readonly()` and re-apply `redact_secrets()` on output (defence-in-depth); `register_mcp_tools(dispatcher, project_root)` iterates all non-denied enabled servers and calls bridge registration; wired into `AgentLoop._build_dispatcher()`; scope="denied" and disabled servers never registered; write/unknown-class tools never registered; per-server failures emit RuntimeWarning and continue. `sac mcp list-native [--json]` lists all registered MCP native tools. 20 tests; full suite passed. |
| `v5.4.1` ✅ | `dev/v4.19` | `src/safecode/mcp/native_bridge.py`, `src/safecode/cli_mcp.py`, `tests/test_mcp_write_execution.py` | MCP write execution flow: `MCPNativeToolBridge(runner, server, allow_write=True)` for scope="write_proposal_required" servers also registers write-class tools with `requires_approval=True` and `audit_event_type="tool_call_mcp_write"`; write tool handler calls `runner.propose_write()` and returns `status="blocked"` with `mcp_write_proposal_id` in metadata (never executes without grant); `_handle_mcp_write_proposal()` helper surfaceable in shell approval flow; `register_mcp_tools()` passes `allow_write=True` for write_proposal_required scope. `sac mcp execute <server> <tool> --grant-id <id> [--input JSON] [--json]`: one-shot CLI that requires a valid MCPApprovalStore grant, calls `MCPReadOnlyRunner.execute_granted_write()`, never executes without grant; audited as `mcp_granted_write_*`. 13 tests; full suite passed. |
| `v5.4.2` ✅ | `dev/v4.19` | `README.md`, `docs/providers.md`, `docs/user-guide.md`, `docs/troubleshooting.md` | MCP depth docs cut: README "MCP Integration (v5.4, EXPERIMENTAL)" section with server config, native tool bridge, write approval flow, and `mcp_<server>_<tool>` naming policy. MVP guide "Extending with MCP Servers" section with sqlite walkthrough. Troubleshooting "MCP Integration Issues" covering tool-not-in-/tools, write-tool-blocked, and name-collision cases. docs/providers.md MCP server configuration reference updated with scope values, native bridge requirements, write proposal flow. |

## v5.5.x: Production Readiness

| 版本 | 分支 | 主要入口 | 验收命令 / 结果 |
|---|---|---|---|
| `v5.5.0` ✅ | `dev/v4.19` | `.github/workflows/ci.yml`, `tests/test_ci_matrix.py` | PyPI release CI: `release` job added to `.github/workflows/ci.yml` triggered on `v*.*.0` tag pushes; `needs: [test]`; `SAFECODE_PUBLISH=1` + `UV_PUBLISH_TOKEN` from secrets; runs `sac release preflight` then `sac release publish --no-dry-run`; `id-token: write` for OIDC Trusted Publishing; `continue-on-error: false` (blocking). 11 new tests in `tests/test_ci_matrix.py`. Full suite: 5328 passed, 4 skipped. |
| `v5.5.1` ✅ | `dev/v4.19` | `scripts/update-brew-formula.sh`, `src/safecode/cli_ops.py`, `tests/test_homebrew_formula.py` | Homebrew formula: `scripts/update-brew-formula.sh <version> <sha256>` generates `Formula/safecode-agent.rb` using pipx shim; `sac release publish --update-brew` runs the script after successful PyPI upload and reports the result; `_run_update_brew_formula()` helper in cli_ops; fails gracefully when script or bash missing. 13 tests in `tests/test_homebrew_formula.py`. Full suite: 5341 passed, 4 skipped. |
| `v5.5.2` ✅ | `dev/v4.19` | `docs/install-update.md`, `docs/current-status.md`, `README.md`, `docs/version_implementation_matrix.md` | Production docs cut: install-update.md adds full install matrix (PyPI/Homebrew/offline/source dev table + per-method install blocks); current-status.md updated to v5.5.2 baseline with distribution and MCP capability tables; README install section rewritten (PyPI first, Homebrew second); README status badge "Production (v5.5)". |
...
| `v4.2.2` ✅ | `main` | `src/safecode/doctor.py`, `README.md`, `docs/user-guide.md`, `docs/troubleshooting.md`, `tests/test_doctor_missing_deps.py` | T-4.2.2-A doctor-missing-deps: `Doctor._project_tooling_diagnostics()` added; no profile → single SKIP "run sac profile detect"; with profile → one diagnostic per kind (test/lint/typecheck/build): PASS (detected+binary present), SKIP (not detected or missing_dependency=True); missing tool → SKIP not FAIL; SKIP includes binary name and sac profile set hint; diagnostic substrate contract preserved. T-4.2.2-B v4.2-docs-cut: README "Profile commands (v4.2)" section; user-guide.md updated to v4.2.x + profile flow section; troubleshooting.md missing-tool section; all v4.2 surfaces EXPERIMENTAL. 30 new tests; full suite 4005 passed, 2 skipped; contract snapshots green. |
...
| `v4.8.2` ✅ | `main` | `README.md`, `docs/user-guide.md`, `docs/public-contracts.md`, `docs/versioning-policy.md`, `docs/security/threat-model-v3.6.md`, `docs/version-notes/v4.8.2-final-v4-shell-first-docs-cut.md` | T-4.8.2-A v4.8-final-docs-cut: README adds Python tutorial link and task-first daily loop summary with 17-command surface and v4.x train closure note; MVP guide updated to v4.8.x with Task-First Daily Loop section; public-contracts adds v4.x series summary (zero new stable contracts v4.0–v4.8); versioning-policy adds v4.x train closure section and v4.8.2 changelog entry; threat-model adds v4.x shell-first addendum table for all new surfaces. Full suite 4276 passed, 2 skipped. |

## v5.6.x–v5.8.x: Agent Quality, Security, Cost, and v6.0 Preparation

| 版本 | 分支 | 主要入口 | 验收命令 / 结果 |
|---|---|---|---|
| `v5.6.0` ✅ | `dev/v4.19` | `src/safecode/agent/prompts.py`, `LICENSE`, `SECURITY.md`, `CONTRIBUTING.md`, `CHANGELOG.md`, `README.md`, `tests/test_system_prompt.py`, `tests/test_project_docs.py` | SYSTEM_PROMPT rewritten from 5 to 8 sections. Root portfolio hygiene files added. README first screen portfolio-ready. 36 tests pass. |
| `v5.6.1` ✅ | `dev/v4.19` | `src/safecode/eval/live.py`, `src/safecode/cli_ops.py`, `.github/workflows/ci.yml`, `tests/test_live_eval_mode.py`, `tests/test_ci_live_lane.py` | LiveEvalRunner with 5 fixtures, --mode live CLI, ratchet baseline, CI job. 56 tests pass. |
| `v5.6.2` ✅ | `dev/v4.19` | `examples/golden-demo/`, `docs/demo/portfolio-demo.md`, `tests/test_golden_demo.py` | Broken calculator → fix → test pass demo. Transcript, portfolio doc. 22 tests pass. |
| `v5.7.0` ✅ | `dev/v4.19` | `docs/security/threat-model-v3.6.md`, `src/safecode/agent/multi_tool_turn.py`, `tests/test_subagent_activation.py`, `tests/test_threat_model_docs.py` | Semi-annual threat model review (6 surfaces). dispatch_parallel for read-only subagents. 40 tests pass. |
| `v5.7.1` ✅ | `dev/v4.19` | `docs/public-contracts.md`, `docs/versioning-policy.md`, `tests/test_public_contract_snapshots.py` | Sandbox execution promoted to stable (Section 17). OTel/HTML frozen experimental. |
| `v5.7.2` ✅ | `dev/v4.19` | `tests/test_project_docs.py`, `tests/test_public_contract_snapshots.py` | CHANGELOG/CONTRIBUTING content checks. Contract test extensions. |
...
| `v5.8.2` ✅ | `dev/v4.19` | `README.md`, `docs/versioning-policy.md`, `docs/public-contracts.md`, `docs/current-status.md` | v6.0 roadmap table, v6 budget in versioning policy, v6 candidate surfaces in public-contracts. Pre-v6 baseline. |

## v6.0.0: Major Contract Cut

| 版本 | 分支 | 主要入口 | 验收命令 / 结果 |
|---|---|---|---|
| `v6.0.0` ✅ | `dev/v4.19` | `pyproject.toml`, `src/safecode/__init__.py`, `README.md`, `docs/public-contracts.md`, `docs/versioning-policy.md`, `docs/v6-contract-candidates.md`, `docs/current-status.md`, `docs/version-notes/v6.0.0-major-contract-cut.md` | Major contract cut: trust mode schema promoted to Section 18, session rollback promoted to Section 19, cost cap deferred, zero v5.0 breaking changes. |

## v6.1.0: Portfolio Maturity Cut

| 版本 | 分支 | 主要入口 | 验收命令 / 结果 |
|---|---|---|---|
| `v6.1.0` ✅ | `dev/v4.19` | `src/safecode/eval/live.py`, `src/safecode/llm/openai_client.py`, `src/safecode/context/diagnostics.py`, `src/safecode/hooks/runner.py`, `examples/realistic-demo/`, `docs/evaluation.md`, `tests/test_live_eval_mode.py`, `tests/test_hooks_mvp.py`, `tests/test_context_diagnostics.py`, `tests/test_realistic_demo.py` | Portfolio maturity: real DeepSeek `deepseek-v4-flash` live eval 2/2 pass, realistic multi-file demo, hooks MVP, diagnostics-aware context, structured provider patch contract. |
...
| `v6.2.2` ✅ | `dev/v4.19` | `README.md`, `docs/public-contracts.md`, `docs/version_implementation_matrix.md`, `docs/user-guide.md`, `.claude/skills/current/SKILL.md`, `.claude/versions.json` | Docs cut: README "Project Memory" section; public-contracts Section 21 candidate; matrix v6.2.x rows; MVP guide memory workflow. |
...
| `v6.3.2` ✅ | `dev/v4.19` | `README.md`, `docs/context-budgets.md`, `docs/user-guide.md`, `docs/version_implementation_matrix.md`, `.claude/skills/current/SKILL.md`, `.claude/versions.json` | Docs cut: README semantic search section; context-budgets hybrid retrieval section; MVP guide search workflow; matrix v6.3.x rows. |
...
| `v6.4.1` ✅ | `dev/v4.19` | `README.md`, `docs/user-guide.md`, `docs/version_implementation_matrix.md`, `.claude/skills/current/SKILL.md`, `.claude/versions.json` | Docs cut: README GitHub PR Workflow section; MVP guide PR workflow; matrix v6.4.x rows. |
...
| 版本 | 分支 | 主要入口 | 验收命令 / 结果 |
|---|---|---|---|
| `v6.6.0` ✅ | `dev/v4.19` | `pyproject.toml`, `src/safecode/index/embedding_backend.py`, `src/safecode/cli_project.py`, `README.md`, `docs/user-guide.md`, `tests/test_index_embedding.py` | Semantic retrieval optional extra `safecode-agent[semantic]`; null backend emits one install-hint warning; `sac index build/status` prints semantic-inactive guidance; targeted embedding tests pass. |
| `v6.6.1` ✅ | `dev/v4.19` | `src/safecode/eval/live.py`, `tests/eval_fixtures/swebench_lite/`, `tests/snapshots/live_eval/baseline.json`, `tests/snapshots/swebench_lite/latest.json`, `docs/demo/eval-methodology.md`, `docs/demo/swebench-eval-summary.md` | Live eval fixtures cover calculator fix, docs edit, multi-file refactor, test failure repair, and config schema migration; SWE-bench Lite synthetic suite expands to 7 inline fixtures; mock baseline is honest 0/7; methodology doc clarifies benchmark scope. |
| `v6.6.2` ✅ | `dev/v4.19` | `examples/real-world-demo/`, `tests/eval_fixtures/swebench_lite/real-world-arrow-parser-error.json`, `tests/snapshots/swebench_lite/latest.json`, `README.md` | Real-world demo scaffold based on Arrow issue #535; reproducer script, demonstration transcript, SWEBenchTask-shaped fixture, and inline SWE-bench Lite adaptation; mock baseline is honest 0/8. |
| `v6.6.3` ✅ | `dev/v4.19` | `README.md`, `docs/user-guide.md`, `docs/public-contracts.md`, `docs/security/threat-model-v3.6.md`, `docs/version_implementation_matrix.md`, `tests/test_mvp_docs.py`, `src/safecode/agent/loop.py`, `src/safecode/cli_shell.py` | Docs cleanup: Trust Modes marked Stable Contract #18, GitHub write workflow marked Stable Contract #20, Project Memory candidate fixed to Section 21, stale README version wording removed, provider/memory/search duplication points to canonical docs, doc guards updated, and noisy runtime version comments reduced. |

## v6.29-v7.0.1: Agent Loop Stabilization and v7 Productization

| 版本 | 分支 | 主要入口 | 验收命令 / 结果 |
|---|---|---|---|
| `v6.29.0` ✅ | `dev/v4.19` | `src/safecode/agent/loop.py`, `src/safecode/agent/loop_*.py`, `tests/test_agent_loop*.py` | AgentLoop layered refactor; large monolithic loop split into focused mixins while preserving behavior. |
| `v6.30.0` ✅ | `dev/v4.19` | `src/safecode/memory/workspace_memory.py`, `src/safecode/agent/loop.py`, `tests/test_workspace_memory.py` | Cross-session `WorkspaceMemoryStore` so future sessions can reuse approved project facts and prior context. |
| `v6.31.0` ✅ | `dev/v4.19` | `src/safecode/eval/live.py`, `tests/test_live_eval_mode.py`, `tests/snapshots/live_eval/baseline.json` | Multi-turn eval fixtures added; default live suite expanded to 36 fixtures; validation-backed multi-turn behavior covered. |
| `v6.32.0` ✅ | `dev/v4.19` | `src/safecode/agent/loop.py`, `tests/test_dynamic_replanning.py` | Dynamic re-planning added with bounded `replan_count` tracking and targeted tests. |
| `v6.33.0` ✅ | `dev/v4.19` | `src/safecode/context/budget.py`, `src/safecode/context/collector.py`, `tests/test_context_back_pressure.py` | Context back-pressure via `ModelContextProfile` and effective context budgets for large-context models. |
| `v6.35.0` ✅ | `dev/v4.19` | `src/safecode/agent/read_tools.py`, `tests/test_search_symbol.py` | `search_symbol` discovery improved with snake/camel matching and better ripgrep fallback behavior. |
| `v7.0.0` ✅ | `dev/v4.19` | `docs/public-contracts.md`, `docs/versioning-policy.md`, `src/safecode/cli_ops.py`, `tests/test_v7_stable_contracts.py` | Stable contract cut: project memory store shape, `search_symbol`, and model context budget profile promoted. 23 total stable contracts; zero v6.0 breaking changes. |
| `v7.0.1` ✅ | `dev/v4.19` | `pyproject.toml`, `src/safecode/__init__.py`, `README.md`, `.claude/versions.json`, `.claude/skills/current/SKILL.md`, `docs/reference/version-summary.md`, `docs/planning-history.md`, `tests/test_docs_governance.py` | Metadata and narrative sync: package/runtime metadata, README/status claims, current planning docs, and docs governance align with v7.0.x, 36 default live fixtures, and 23 stable contracts. |
| `v7.0.2` ✅ | `dev/v4.19` | `src/safecode/cli_agent.py`, `tests/test_cli_agent_run.py`, `README.md`, `docs/version-notes/v7.0.2-single-command-agent-run.md` | Single-command agent happy path: `sac agent run` supports `--auto-edit`, `--full-auto`, and `--tests`; JSON output includes `tools_used`, `files_changed`, `validation`, and `rollback_command`; human summary mirrors those fields. Approval gates remain unchanged. |
| `v7.0.3` ✅ | `dev/v4.19` | `src/safecode/subagents/roles.py`, `src/safecode/cli_subagent.py`, `tests/test_cli_subagent_roles.py`, `README.md`, `docs/version-notes/v7.0.3-productized-subagent-roles.md` | Productized read-only subagent roles: `roles`, `explore`, `review`, and `scout` commands; role outputs journaled as `subagent_dispatch` events for main-agent context merge; write-capable subagents remain out of scope. |
| `v7.0.4` ✅ | `dev/v4.19` | `src/safecode/report/session_timeline.py`, `src/safecode/cli_session.py`, `src/safecode/cli_shell.py`, `tests/test_cli_session_views.py`, `tests/test_shell_session_quality.py`, `docs/version-notes/v7.0.4-session-shell-polish.md` | Session and shell polish: human timeline projection from agent journals, `sac session show --timeline`, `sac session resume`, shell `/timeline`, `/sessions`, and `/resume <id>`. Timeline JSON remains experimental. |
| `v7.1.0` ✅ | `dev/v4.19` | `src/safecode/eval/live.py`, `src/safecode/cli_ops.py`, `tests/test_live_eval_mode.py`, `docs/demo/eval-methodology.md`, `docs/version-notes/v7.1.0-eval-suite-split.md` | Eval suite split: live fixtures are classifiable/filterable as `regression`, `capability`, `safety`, or `cost-perf`; `sac eval --mode live --eval-suite <suite>` selects a slice. |
| `v7.1.1` ✅ | `dev/v4.19` | `src/safecode/eval/live.py`, `src/safecode/cli_ops.py`, `tests/test_live_eval_mode.py`, `docs/demo/eval-methodology.md`, `docs/version-notes/v7.1.1-live-eval-transcripts.md` | Opt-in redacted live eval transcripts via `sac eval --mode live --transcripts`; runner writes per-fixture JSON artifacts with fixture metadata, trajectory milestones, validation commands, and outcome metrics. |
| `v7.1.2` ✅ | `dev/v4.19` | `src/safecode/eval/live.py`, `tests/test_live_eval_mode.py`, `docs/version-notes/v7.1.2-multi-grader-schema.md` | Multi-grader schema: live eval results now include structured grader outcomes for fixture outcome, validation, safety invariants, scope control, and reviewer-gate quality. |
| `v7.1.3` ✅ | `dev/v4.19` | `src/safecode/eval/live.py`, `tests/test_live_eval_mode.py`, `tests/snapshots/live_eval/baseline.json`, `docs/version-notes/v7.1.3-balanced-negative-evals.md` | Balanced negative evals: default live suite expanded to 38 fixtures with safety cases that check no shell/subprocess for simple fixes and docs-only typo repair without code churn. |
| `v7.1.4` ✅ | `dev/v4.19` | `src/safecode/eval/real_task.py`, `src/safecode/cli_ops.py`, `tests/test_eval_real_task_benchmark.py`, `docs/version-notes/v7.1.4-real-task-benchmark.md` | Real-task benchmark lane: `sac eval --mode real-task` selects external-project SWE-bench-Lite-compatible tasks, renders benchmark provenance, and saves `.sac/eval/real-task/latest.json`. |
| `v7.1.5` ✅ | `dev/v4.19` | `src/safecode/eval/dashboard.py`, `src/safecode/cli_ops.py`, `tests/test_eval_report_dashboard.py`, `docs/version-notes/v7.1.5-eval-report-dashboard.md` | Eval report dashboard: `sac eval --mode dashboard` summarizes latest live, SWE-bench Lite, and real-task JSON reports into Markdown at `.sac/eval/dashboard.md`. |
