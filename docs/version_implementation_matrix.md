...
| `v4.10.1` ✅ | `main` | `src/safecode/cli_setup.py`, `src/safecode/llm/deepseek.py`, `tests/test_setup_wizard_deepseek.py`, `docs/version-notes/v4.10.1-deepseek-setup-wizard.md` | T-4.10.1-A deepseek-setup-wizard: `sac setup --wizard` gains `deepseek` preset option; wizard resolves `DEEPSEEK_API_KEY` from env, writes `base_url` and `default_model` from preset, prints confirmation panel; existing openai/openai-compatible wizard paths unaffected; no auto-write without interactive selection. All new surfaces EXPERIMENTAL. Targeted tests pass; full suite clean. |
| `v4.10.2` ✅ | `main` | `src/safecode/doctor.py`, `tests/test_doctor_provider_diagnostics.py`, `docs/version-notes/v4.10.2-provider-doctor-diagnostics.md` | T-4.10.2-A provider-doctor-diagnostics: `Doctor._provider_diagnostics()` checks API key env var present, `base_url` non-empty, model name non-empty, network policy permits base URL; makes no live provider network request; PyPI update check gated by `SAFECODE_OFFLINE=1`; all diagnostics report PASS/SKIP/FAIL with actionable hints. All new surfaces EXPERIMENTAL. Targeted tests pass; full suite clean. |
| `v4.10.3` ✅ | `main` | `src/safecode/llm/openai_client.py`, `src/safecode/config.py`, `tests/test_llm_retry_handling.py`, `docs/version-notes/v4.10.3-provider-retry-reliability.md` | T-4.10.3-A provider-retry-reliability: `OpenAICompatibleLLMClient` gains rate-limit-aware retry on HTTP 429/5xx (bounded exponential backoff, default 3 retries), configurable per-call timeout (default 60 s), structured progress callback; `llm.max_retries` and `llm.timeout_seconds` config fields added (bounded, optional); no retry on 4xx; secrets never logged during retry. All new surfaces EXPERIMENTAL. Targeted tests pass; full suite clean. |
| `v4.10.4` ✅ | `main` | `src/safecode/cli_smoke.py`, `tests/test_smoke_live_provider.py`, `docs/version-notes/v4.10.4-live-provider-smoke.md` | T-4.10.4-A live-provider-smoke: `sac smoke live-provider` opt-in scenario gated by `SAFECODE_SMOKE_LIVE=1`; makes real provider network call, verifies minimal chat completion, records round-trip latency; hidden from `sac --help`; existing mock-only smoke scenarios unaffected. All new surfaces EXPERIMENTAL. Targeted tests pass; full suite clean. |
