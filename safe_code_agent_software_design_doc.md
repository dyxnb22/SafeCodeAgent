...
最新一轮安全检查继续追加 `v1.5.15` 到 `v1.5.19`，覆盖 git include/clean 旁路、审批解析容错与版本绑定、anchor 信任边界、token redaction、以及 apply 的 symlink/identity race 防护。只有 `v1.5.15` 到 `v1.5.19` 的测试全部通过后，才允许开始 v1.6 的 MCP 真执行与 subagent 扩展。
