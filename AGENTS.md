...
The planning documents indexed by `product-planning/README.md` and
`enterprise-docs/README.md` are the delivery source of truth. Foundation
overviews are background only. Legacy product documentation belongs on `main`
or `archive/safecodeagent-final`; do not recreate it here.
...
- If active instructions conflict with a safety invariant or authoritative plan,
...
5. Only the authoritative plan documents, code, callers, and tests routed for
   the active task.

Do not read every planning document or scan the whole repository by default.
...
3. Identify the task ID, acceptance criteria, affected feature, owning module,
   architecture layer, trust boundary, and reused kernel components.
4. Read the relevant authoritative plan, implementation, callers, and tests.
...
- Keep changes scoped to the active milestone and dependency chain.
...
- Do not implement later roadmap stages speculatively.
...
3. Run the full suite before a milestone or when the task requests completion.
4. Inspect all modified and untracked files and remove task-created temporary
   output.
5. Reconcile implementation with acceptance criteria and record real conflicts;
...
`.agents/context/progress.json` is the only live progress source. Planning prose
describes intended work; it does not prove completion.
...
- On completion, require acceptance evidence and relevant tests, append the task
...
- Never mark a roadmap task complete based only on generated code, a summary, or
  passing narrow tests when broader acceptance is required.
...
- Architecture, feature, roadmap, decision, testing, and release documents are
  updated only when their corresponding facts change, not after every task.
...
- Update an authoritative document only when its contract, decision, acceptance
  criterion, or implementation status actually changes.
...
- Preserve backward compatibility in the reusable kernel unless an approved
  plan explicitly changes the contract.
...
A milestone is complete only when its acceptance criteria are implemented, its
targeted tests pass, relevant regression tests pass, temporary artifacts are
removed, and authoritative planning status is accurate. Closeout is a gate, not
a new family of summary documents or a reason to delete durable regression
coverage.
